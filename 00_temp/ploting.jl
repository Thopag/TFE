export init_plot_multi_simulation, plot_single_simulation, plot_multi_simulation, plot_parameter_analyses, NaV_palettes
...
function plot_multi_simulation(plt, sol, current, p_model, t_spikes, color; label="")
    ...
    voltage = 1
    ylabel!(plt[voltage], "Voltage (mV)", legend = :topright, ylims=(-90,50), legendfontsize=9)
...
function plot_parameter_analyses(VEC_intra_parameter, VEC_inter_parameter, 
    ...
    xticks = VEC_intra_parameter[1]:30:VEC_intra_parameter[end]

    p_peaks = plot(xlabel=intra_axe_label, ylabel= "Peaks count (-)", xticks=xticks)
    p_freqs = plot(xlabel=intra_axe_label, ylabel= "Frequence (Hz)", xticks=xticks)
    p_height = plot(xlabel=intra_axe_label, ylabel= "Height (mV)", xticks=xticks)
    p_width = plot(xlabel=intra_axe_label, ylabel= "width (ms)", xticks=xticks)
    ...
    p_pattern = plot(xlabel=intra_axe_label, ylabel=inter_axe_label, yticks = (1:length(inter_labels), inter_labels), xticks=xticks, legend=:topright, legendfontsize=7)
    ...
    # --- first peak height --- #
    hline!(p_height, [-5], color=:red, linestyle = :dash, label="")
